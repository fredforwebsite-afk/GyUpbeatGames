...
import {
  getFirestore, doc, setDoc, updateDoc, getDoc, onSnapshot, arrayUnion, arrayRemove
...
async function setStateField(field, value) {
  try {
    await updateDoc(stateDoc, { [field]: value });
  } catch (err) {
    // if update fails (e.g., doc missing), set with merge
    await setDoc(stateDoc, { [field]: value }, { merge: true });
  }
}
...
async function updateScoreForTeam(team, delta) {
  ...
  await updateDoc(scoresDoc, { [team]: newScore }).catch(async () => {
    // fallback to set
...
async function setTeamAnswer(team, answer) {
  try {
    await updateDoc(answersDoc, { [team]: answer });
  } catch (err) {
    await setDoc(answersDoc, { [team]: answer }, { merge: true });
  }
}
